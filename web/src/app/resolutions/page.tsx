...
function CommitteePanel({ committee }: { committee: Committee }) {
  const entries = resolutions[committee.slug] ?? [];
          ...
          <ul>
            {committee.topics.map((topic, i) => (
              <TopicRow
                ...
                index={i}
                title={topic.title}
                entry={entries.find((e) => e.topicIndex === i)}
              />
            ))}
          </ul>
        </div>
...
export default function ResolutionsPage() {
  const sections = committees.map((committee) => ({
    ...
    content: <CommitteePanel committee={committee} />,
  }));
