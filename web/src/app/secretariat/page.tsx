...
import { committees, secretariat, type Person } from "@/lib/conference";
...
export default function SecretariatPage() {
  ...
  const chairSections = (["ecosoc", "unoosa"] as const).map((slug) => ({
    slug,
    committee: committees.find((c) => c.slug === slug),
    people: chairs[slug],
  }));
        ...
        <>
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            {executives.map((person) => (
              <ExecutiveCard key={person.name} person={person} />
            ))}
          </div>
        </>
      ),
